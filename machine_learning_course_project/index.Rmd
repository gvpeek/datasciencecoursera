...
## Summary
In the following analysis, I am going to attempt to create a model which can predict the manner in which a subject performs a barbell lift, based on data from six accelerometers worn during the exercise. A training set was provided to build the model. A test set was also provided, but with the correct predictions withheld, as it will be used to grade the efficacy of the models. Therefore, it cannot be used to validate the models in this analysis.
...
On first examination of the training dataset, seeing that it has some properties of a time series, I was uncertain if this prediction would need to be treated as a forecast or not. Since our predictions involve classification which will not change over the course of a particular lift, I felt it better to approach each measurement as an observation not temporally linked with its neighbor.
